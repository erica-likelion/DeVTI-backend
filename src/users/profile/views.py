...
PART_DISPATCHER = {
    "PM": {
        "serializer": ProfilePMSerializer,
        "model": ProfilePM,
        "message": "PM 프로필 생성 완료",
    },
    "FE": {
        "serializer": ProfileFESerializer,
        "model": ProfileFE,
        "message": "FE 프로필 생성 완료",
    },
    "BE": {
        "serializer": ProfileBESerializer,
        "model": ProfileBE,
        "message": "BE 프로필 생성 완료",
    },
    "DE": {
        "serializer": ProfileDESerializer,
        "model": ProfileDE,
        "message": "DE 프로필 생성 완료",
    },
}
...
def participant_profile_view(request, participant_id):
    ...
    common_profile = get_object_or_404(Profile, user=target_user)

    try:
        part_profile = PartModel.objects.get(profile=common_profile)
...
class ProfileView(APIView):
    def get(self, request):
        ...
        available_parts = []
        if ProfilePM.objects.filter(profile=profile).exists():
            available_parts.append("PM")
        if ProfileFE.objects.filter(profile=profile).exists():
            available_parts.append("FE")
        if ProfileBE.objects.filter(profile=profile).exists():
            available_parts.append("BE")
        if ProfileDE.objects.filter(profile=profile).exists():
            available_parts.append("DE")
        ...
        if not part:
            ...
            if part not in PART_DISPATCHER:
                raise ParseError(
                    "part 쿼리 파라미터가 유요하지 않습니다. (허용 값: PM, FE, BE, DE)"
                )
            ...
            try:
                part_instance = PartModel.objects.get(profile=profile)
            except PartModel.DoesNotExist:
    ...
    def post(self, request):
        ...
        if PartModel.objects.filter(profile=profile).exists():
        ...
        try:
            with transaction.atomic():
                if not part:  # part 파라미터가 없는 경우(공통 프로필 수정)
                    ...
                    if part not in PART_DISPATCHER:
                        raise ParseError(
                            "part 쿼리 파라미터가 유효하지 않습니다. (허용 값: PM, FE, BE, DE)"
                        )
                    ...
                    try:
                        ...
                        raise NotFound(
                            f"해당 파트({part})의 프로필이 존재하지 않습니다. (POST로 생성 필요)"
                        )

                    serializer = PartSerializer(
                        part_instance, data=request.data, partial=True
                    )
                    serializer.is_valid(raise_exception=True)
                    serializer.save()
                    ...
                    part_data = (
                        serializer.data
                    )  # 수정한 part_data 응답에 추가로 넘기기 위해 변수에 담아놓기
